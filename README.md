...
また、ris- で始まる各種リポジトリ（例: ris-bot-discord, ris-infra-core など）は、インフラ・開発共通の基盤として ris-infra-core リポジトリを中心にREADMEや構成、PRテンプレート、ビルド用シェルスクリプト等を管理しています。
...
`bots/local/` ディレクトリには、デーモン化して常時稼働させるタイプのBotを実装・運用します。
...
- プッシュ・PR作成時に、コードや設定ファイル内にDiscordのBotトークン等の機密情報が含まれていないか自動チェックを行います。
  - 例: `DISCORD_TOKEN` などの文字列や形式を検出し、漏洩を未然に防止します。
- その他のテストやビルドは現状実施していません。
